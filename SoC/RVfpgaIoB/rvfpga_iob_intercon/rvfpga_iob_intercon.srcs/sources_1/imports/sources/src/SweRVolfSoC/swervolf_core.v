...
`include "axi_interconnect_iob.vh"
...
axi_interconnect_iob #(
  ...
  .S_COUNT      (NoSlaves),
  .M_COUNT      (NoMasters),
  .M_BASE_ADDR  ({{32'h80000000}, {32'h00000000}})
//  .M_ADDR_WIDTH ({{32'd8}, {32'd24}})
) axi_intercon (
  .clk(clk),
  .rst(rst_n),
...
//  .s_axi_awregion({4'd0, lsu_awregion, sb_awregion}),
  ...
  .s_axi_wready({o_ifu_wready, lsu_wready, sb_wready}),
  
  .s_axi_bid({o_ifu_bid, lsu_bid, sb_bid}),
//  .s_axi_bid({o_ifu_bid, o_lsu_bid, o_sb_bid}),
...
//  .s_axi_rid({o_ifu_rid, lsu_rid, sb_rid}),
