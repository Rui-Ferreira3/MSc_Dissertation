    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_N2_DATA_0: begin
                    rdata <= int_N2[31:0];
                end
                ADDR_N3_DATA_0: begin
                    rdata <= int_N3[31:0];
                end
            endcase
        end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_N1_DATA_0)
            int_N1[31:0] <= (WDATA[31:0] & wmask) | (int_N1[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_N2_DATA_0)
            int_N2[31:0] <= (WDATA[31:0] & wmask) | (int_N2[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_N3_DATA_0)
            int_N3[31:0] <= (WDATA[31:0] & wmask) | (int_N3[31:0] & ~wmask);
    end
end
