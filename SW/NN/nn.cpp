...
int main(int argc, const char * argv[]) {
    ...
    vector <float> W1_best;
    vector <float> W2_best;
    vector <float> W3_best;
    float loss_best = 1.0;
    ...
    for (unsigned i = 0; i < 10000; ++i) {
        ...
        if ((i+1) % 100 == 0){
            ...
            if (loss/BATCH_SIZE < loss_best) {
                loss_best = loss/BATCH_SIZE;
                W1_best = W1;
                W2_best = W2;
                W3_best = W3;
            }
    ...
    ofstream outfile ("data/weights.txt");
    outfile << "float W1[] = [";
    for(const float &val : W1_best) {
        outfile << val << ", ";
    }
    outfile << "];" << endl;
    outfile << "float W2[] = [";
    for(const float &val : W2_best) {
        outfile << val << ", ";
    }
    outfile << "];" << endl;
    outfile << "float W3[] = [";
    for(const float &val : W3_best) {
        outfile << val << ", ";
    }
    outfile << "];" << endl;
    outfile.close();
